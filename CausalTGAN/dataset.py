...
class PlainTransformer(object):
    ...
    def _fit_discrete(self, column_name, raw_column_data):
        """Fit one hot encoder for discrete column."""
        le = LabelEncoder()
        le.fit(raw_column_data.reshape(-1, 1))
        num_categories = len(list(le.classes_))

        return ColumnTransformInfo(
            column_name=column_name, column_type="discrete", transform=le,
            transform_aux=None,
    ...
    def _transform_discrete(self, column_transform_info, raw_column_data):
        ...
        encoded_column = le.transform(raw_column_data).reshape(-1, 1)
    ...
    def _inverse_transform_discrete(self, column_transform_info, column_data):
        le = column_transform_info.transform
        column_data = np.round(column_data).astype('int')
        column_data = np.clip(column_data, 0, len(le.classes_)-1)
        return le.inverse_transform(column_data)
